...
const Navbar = () => {
  const [activeItem, setActiveItem] = useState(sessionStorage.getItem('activeNavItem') ?? 'home');
  ...
  // Load activeItem from sessionStorage on mount
  useEffect(() => {
    const storedItem = sessionStorage.getItem('activeNavItem');
    if (storedItem) setActiveItem(storedItem);
  }, []);

  // Save activeItem to sessionStorage when it changes
  useEffect(() => {
    sessionStorage.setItem('activeNavItem', activeItem);
  }, [activeItem]);
